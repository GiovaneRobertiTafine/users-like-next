...
export default function BoxFollowers({ unfollowUser, viewThumb }: BoxFollowersComponent) {
    ...
    useEffect(() => {
        window.addEventListener('resize', (event) => {
            ...
            if (window.innerWidth > 400 || !viewFollowers) {
        ...
        if (followersRef && !followersSubject$.value) {
            followersSubject$.next([...followersRef.map((v) => JSON.parse(v))]);
        } else { followersSubject$.next([]); }
                                ...
                                <div className="box-list-followers">
                                    <h4>
                                        ...
                                        <strong style={{ color: '#000', marginLeft: '10px', cursor: 'pointer' }}
                                            onClick={() => { setViewFollowers(false); document.documentElement.style.overflow = 'auto'; }}>X</strong>
                                    </h4>
                                    <div className="list-followers">
                                        {
                                            followers.map((v) => {
                                                return (
                                                    <div className="item-list-followers" key={v.login.uuid} onClick={() => viewThumb(v)}>
                                                        ...
                                                        <button className="btn-unfollow" onClick={(event) => { event.stopPropagation(); unfollowUser(v.login.uuid); }}>unfollow</button>
                                                    </div>
                                                );
                                            })
                                        }
